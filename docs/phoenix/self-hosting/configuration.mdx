...
* **PHOENIX\_POSTGRES\_USE\_AWS\_IAM\_AUTH:** Enable AWS RDS IAM database authentication. When enabled, Phoenix uses AWS IAM credentials to generate short-lived tokens instead of static passwords. See [Using Amazon Aurora](/docs/phoenix/self-hosting/configuration/using-amazon-aurora) for details.

* **PHOENIX\_POSTGRES\_USE\_AZURE\_AD\_AUTH:** Enable Azure AD (Microsoft Entra ID) authentication for Azure Database for PostgreSQL. When enabled, Phoenix uses Azure credentials to obtain JWT tokens for database authentication. See [Using Azure Database](/docs/phoenix/self-hosting/configuration/using-azure-database) for details.

* **PHOENIX\_AZURE\_CLIENT\_ID:** Client ID for user-assigned managed identity when using Azure AD authentication. Only needed for user-assigned managed identity; system-assigned managed identity and service principals are auto-detected.
